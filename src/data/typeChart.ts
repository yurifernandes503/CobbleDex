...
export function getWeaknesses(types: PokemonType[]): PokemonType[] {
  ...
  return allTypes.filter((atk) => getEffectiveness(atk, types) >= 2);
}
...
export function getResistances(types: PokemonType[]): PokemonType[] {
  ...
  return allTypes.filter((atk) => getEffectiveness(atk, types) > 0 && getEffectiveness(atk, types) < 1);
}
